...
import { CheckOutlined, CloseOutlined, CreditCardOutlined, CrownOutlined, LockOutlined, ReloadOutlined, UnlockOutlined } from '@ant-design/icons';
import { useAuth } from '../src/context/AuthContext';
import { getPlatformSnapshot } from './superadminService';
import { activateManualSubscription, approveManualRequest, confirmManualPayment, formatPrice, getTierById, getSuggestedTier, PACKAGE_TIERS, rejectManualRequest, setManualAccessRestriction, subscribeManualRequests, subscriptionStatus } from './superadminSubscriptionService';
...
export default function SuperAdminSubscriptions() {
  ...
  return <div style={{maxWidth:1500,margin:'0 auto'}}>
    ...
    {tab==='google' && <Card style={{...card,marginTop:16}} title="Google Play / RevenueCat"><Table rowKey="id" columns={subscriptionColumns} dataSource={googleRows} pagination={{pageSize:20}} locale={{emptyText:'Google Play aboneliği bulunamadı.'}}/></Card>}
    ...
    <Modal open={!!modal} title={`${modal?.row ? nameOf(modal.row) : ''} — Manuel / IBAN Abonelik`} okText="Aktif Et" cancelText="Vazgeç" confirmLoading={busy} onOk={saveManual} onCancel={()=>!busy&&setModal(null)} destroyOnHidden width={560}>
