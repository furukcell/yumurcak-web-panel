...
function collectCredentials() {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if ((lines[i] === 'Yeni' || lines[i] === 'Mevcut') && lines[i + 1]?.includes(' / ')) {
      ...
      if (kullaniciAdi && sifre && role) rows.push({ role, kullaniciAdi, sifre, yeniMi: lines[i] === 'Yeni' });
    }
  }
  return rows;
}
...
export default function SuperAdminCredentialsPdf() {
  const pdfAktar = () => {
    const rows = collectCredentials().filter((x) => x.yeniMi);
    if (!rows.length) {
      message.info('PDF için yeni oluşturulmuş kullanıcı hesabı bulunamadı. Önce toplu kurulumu tamamlayın.');
      return;
    }
    ...
    const tablo = (baslik, liste) => liste.length ? `
      <section class="section">
        <h2>${baslik}</h2>
        <table>
          <thead><tr><th>Kullanıcı Adı</th><th>Şifre</th></tr></thead>
          <tbody>${liste.map((x) => `<tr><td class="mono">${escapeHtml(x.kullaniciAdi)}</td><td class="mono">${escapeHtml(x.sifre)}</td></tr>`).join('')}</tbody>
        </table>
      </section>` : '';
    ...
    popup.document.write(`<!doctype html><html lang="tr"><head><meta charset="utf-8" /><title>Yumurcak - Kullanıcı Bilgileri</title>
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        <div class="header"><div class="brand">YUMURCAK</div><div class="title">Kullanıcı Bilgileri</div><div class="meta"><strong>Kurum:</strong> ${escapeHtml(kurum)} &nbsp; • &nbsp; <strong>Tarih:</strong> ${escapeHtml(tarih)}</div></div>
  ...
  return <Button icon={<PrinterOutlined />} onClick={pdfAktar}>Kullanıcı Bilgilerini PDF'e Aktar</Button>;
}
