...
export default function SuperAdminAnalytics() {
  ...
  useEffect(() => {
    Promise.all([getPlatformSnapshot(), getUsageLogs()])
      .then(([s, raw]) => { const normalized = normalizeUsageLogs(raw); setSnapshot(s); setLogs(normalized); const years = getAvailableUsageYears(normalized); if (years.length) setYear(years[0]); })
  ...
  const availableYears = useMemo(() => getAvailableUsageYears(filtered), [filtered]);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i -= 1) {
      ...
      const dayLogs = filtered.filter((x) => Number(x.timestamp) >= from && Number(x.timestamp) < from + DAY);
      rows.push({ key: start.toISOString().slice(0, 10), label: start.toLocaleDateString('tr-TR', { day: '2-digit', month: 'short' }), events: dayLogs.length, users: new Set(dayLogs.map((x) => x.kullaniciId).filter(Boolean)).size, institutions: new Set(dayLogs.map((x) => x.kresId).filter(Boolean)).size });
    }
    return rows;
  }, [filtered]);
  const trendMax = Math.max(1, ...trend.map((x) => x.events));

  const yearlyMonthly = useMemo(() => monthlyUsageSeries(filtered, year), [filtered, year]);
  const monthlyMax = Math.max(1, ...yearlyMonthly.map((x) => x.events));
  const selectedYearTotal = useMemo(() => yearlyMonthly.reduce((sum, x) => sum + x.events, 0), [yearlyMonthly]);
  const selectedYearUsers = useMemo(() => new Set(filtered.filter((x) => { const ts = Number(x.timestamp); return ts > 0 && new Date(ts).getFullYear() === year; }).map((x) => x.kullaniciId).filter(Boolean)).size, [filtered, year]);
  const selectedYearInstitutions = useMemo(() => new Set(filtered.filter((x) => { const ts = Number(x.timestamp); return ts > 0 && new Date(ts).getFullYear() === year; }).map((x) => x.kresId).filter(Boolean)).size, [filtered, year]);

  const users = useMemo(() => {
    const map = {};
    filtered.forEach((x) => {
      const id = x.kullaniciId || x.userId; if (!id) return;
      if (!map[id]) map[id] = { id, events: 0, last: 0, today: 0, last7: 0, modules: new Set(), name: x.kullaniciAdi || x.userName || id, role: x.rol || x.role || '—' };
      map[id].events += 1; map[id].last = Math.max(map[id].last, Number(x.timestamp || 0));
  ...
  const institutionRows = useMemo(() => {
    const map = {}, userSets = {}, lastByInstitution = {}, moduleSets = {};
    filtered.forEach((x) => {
      const id = x.kresId; if (!id) return;
      map[id] = (map[id] || 0) + 1;
      if (!userSets[id]) userSets[id] = new Set(); if (x.kullaniciId) userSets[id].add(x.kullaniciId);
      if (!moduleSets[id]) moduleSets[id] = new Set(); if (x.modul || x.module) moduleSets[id].add(x.modul || x.module);
      lastByInstitution[id] = Math.max(lastByInstitution[id] || 0, Number(x.timestamp || 0));
    });
    return (snapshot?.institutions || []).map((institution) => {
      const id = institution.id, events = map[id] || 0, activeUsers = userSets[id]?.size || 0, totalUsers = snapshot?.usersByKres?.[id] || 0;
      const usage = totalUsers ? Math.min(100, Math.round(activeUsers / totalUsers * 100)) : (events ? 100 : 0);
      const score = Math.min(100, Math.round(usage * .55 + Math.min(events, 100) * .25 + (lastByInstitution[id] && Date.now() - lastByInstitution[id] < 7 * DAY ? 20 : 0)));
      return { id, name: getInstitutionName(institution, id), events, activeUsers, totalUsers, usage, score, last: lastByInstitution[id] || 0, modules: moduleSets[id]?.size || 0 };
  ...
  const trialRows = useMemo(() => (snapshot?.institutions || []).map((institution) => {
    const id = institution.id; const sub = snapshot?.subscriptionsByKres?.[id]; const info = getTrialInfo(sub, logs.filter((x) => x.kresId === id));
    return info ? { id, name: getInstitutionName(institution, id), ...info } : null;
  }).filter(Boolean).filter((x) => kresId === 'all' || x.id === kresId).sort((a, b) => Number(a.ended) - Number(b.ended) || a.remainingDays - b.remainingDays), [snapshot, logs, kresId]);
