...
import { DashboardOutlined, ApartmentOutlined, BarChartOutlined, CrownOutlined, LogoutOutlined } from '@ant-design/icons';
...
export default function SuperAdminLayout() {
  ...
  const menu = {
    items: [{ key: 'cikis', icon: <LogoutOutlined />, label: 'Çıkış Yap' }],
    onClick: async ({ key }) => { if (key === 'cikis') await cikisYap(); },
  };
    ...
    <Layout style={{ minHeight: '100vh', background: '#F7F8FC' }}>
      <Sider width={248} theme="dark" style={{ background: '#17152A' }}>
        <div style={{ height: 76, padding: '0 22px', display: 'flex', alignItems: 'center', gap: 11 }}>
          ...
          <div>
            <Text strong style={{ color: '#fff', fontSize: 18, display: 'block' }}>Yumurcak</Text>
            <Text style={{ color: '#A9A5BE', fontSize: 11 }}>Platform Yönetimi</Text>
          </div>
        </div>
        <Menu
          theme="dark"
          mode="inline"
          selectedKeys={[location.pathname === '/superadmin/' ? '/superadmin' : location.pathname]}
          items={items}
      ...
      <Layout>
        <Header style={{ background: '#fff', borderBottom: '1px solid #ECECF2', padding: '0 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <Text strong style={{ fontSize: 16 }}>SuperAdmin</Text>
            <Text type="secondary" style={{ marginLeft: 10, fontSize: 12 }}>Yumurcak Platform</Text>
          </div>
          <Dropdown menu={menu} placement="bottomRight">
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, cursor: 'pointer' }}>
              <Avatar style={{ background: '#6C3DEB' }}>{String(kullanici?.adSoyad || kullanici?.ad || 'S').charAt(0).toUpperCase()}</Avatar>
              <div style={{ lineHeight: 1.2 }}>
                <Text strong style={{ display: 'block', fontSize: 13 }}>{kullanici?.adSoyad || kullanici?.ad || 'SuperAdmin'}</Text>
                <Text type="secondary" style={{ fontSize: 11 }}>SuperAdmin</Text>
              </div>
            </div>
          </Dropdown>
        </Header>
